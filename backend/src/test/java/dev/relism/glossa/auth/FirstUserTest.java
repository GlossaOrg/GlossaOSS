...
class FirstUserTest {
    ...
    void theFirstAccountIsCreatedOnceAndAdministers() {
        ...
        // The address is checked by the constraint the body's own type declares, before the service runs.
        local.request().json("{\"email\":\"dev\",\"password\":\"Long-enough!\"}").post("/api/setup")
                .expectStatus(422).expectBodyContains("email is not an email address");
