...
public final class UserService {
    ...
    /** A row of the user table: an account, or an invitation not taken up yet. */
    public record AccountView(long id, boolean invitation, String name, String email, List<String> sources, boolean administrator,
                              Status status, boolean passwordReset, Instant createdAt, Instant expiresAt, List<Membership> projects) {}

    public record Membership(long project, String name, Role role, String locale) {}

    /** What an administrator asks for; {@code expiresInDays} of 0 never expires. */
    public record InviteRequest(String email, String name, boolean sso, Long project, Role role, String locale, int expiresInDays) {}

    /** The link is in this answer and nowhere else — like an API key's token. */
    public record Invited(long id, String email, String link) {}

    /** What the invited person is shown before choosing a password. */
    public record InviteView(String email, String name, boolean reset) {}

    /** Who the account turned out to be, so the invitee's browser can sign in with what they just chose. */
